...
getAverageFreqBands <- function(group, band, erps){
...
plotAverageFreqBands <- function(target='inline', bands = c('low_alpha', 'high_alpha', 'low_beta', 'high_beta'), groups = c('rot', 'mir', 'rdm'), erps){
  ...
  for(band in bands){
    ...
    if (erps == 'lrp'){
      ...
      if (target=='svg') {
        svglite(file=sprintf('doc/fig/Fig15_AvgTFR_%s_%s.svg', band, erps), width=12, height=7, pointsize=14, system_fonts=list(sans="Arial"))
      }
    ...
    for(group in groups){
      data <- getAverageFreqBands(group = group, band = band, erps = erps)
      timepts <- data$time
      if(erps == 'lrp'){
