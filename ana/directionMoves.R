...
# Right(C3-C4) - Left(C3-C4)
getEarlyLateLRP <- function(groups = c('aln', 'alnrot', 'alnmir', 'alnearlyrdm', 'alnlaterdm', 'rot', 'rdm', 'mir'), directions = c('right', 'left')){
  ...
  for(group in groups){
    ...
    if(group == 'aln'){
      ...
      write.csv(groupLRP, file=sprintf('data/Blocked_LRP_DF_EarlyLate_%s.csv', group), row.names = F) 
    } else if (group == 'alnearlyrdm' | group == 'alnlaterdm'){
      for(direction in directions){
...
getEarlyLateLateralizedCI <- function(groups = c('aln', 'alnrot', 'alnmir', 'alnearlyrdm', 'alnlaterdm', 'rot_b0', 'rot_b1', 'rdm_b0', 'rdm_b1', 'mir_b0', 'mir_b1'), type = 'b'){
  for (group in groups){
...
plotEarlyLateLateralized <- function(perturbs = c('rot', 'rdm0', 'rdm1', 'mir'), target='inline') {
  ...
  for(ptype in perturbs){
    ...
    if(ptype == 'rot'){
      ...
      #add legend
      legend(-1,-5,legend=c('Aligned','Early ROT', 'Late ROT'),
      ...
      #add legend
      legend(-1,-5,legend=c('Aligned','Early MIR', 'Late MIR'),
             col=c(colourscheme[['aligned']][['S']],colourscheme[['early']][['S']],colourscheme[['late']][['S']]),
             lty=1,bty='n',cex=1,lwd=2)
...
getEarlyLatevsALignedLRPDiffWaves <- function(perturbs = c('rot', 'rdm0', 'rdm1', 'mir')) {
  ...
  for(ptype in perturbs){
    if (ptype == 'rot'){
      ...
      groups = c('rdm_b0')
      adata <- read.csv(file='data/Blocked_LRP_DF_EarlyLate_alnearlyrdm.csv')
    } else if (ptype == 'rdm1'){
      groups = c('rdm_b1')
      adata <- read.csv(file='data/Blocked_LRP_DF_EarlyLate_alnlaterdm.csv')
    }
...
plotSmallLargeLateralized <- function(perturbs = c('rot', 'rdm', 'mir'), target='inline') {
  ...
  for(ptype in perturbs){
    ...
    if(ptype == 'rot'){
      groups = c('alnrot', 'rot_sml', 'rot_lrg')
      # create plot
      meanGroupReaches <- list() #empty list so that it plots the means last
      ...
      #add legend
      legend(-1,-5,legend=c('Aligned','Small ROT', 'Large ROT'),
      ...
      abline(h = c(0), v = c(0), col = 8, lty = 2) #creates horizontal dashed lines through y =  0 and 30
      axis(1, at = c(-1.0, -0.5, -0.25, 0, 0.25, 0.5)) #tick marks for x axis
      axis(2, at = c(-15, -10, -5, 0, 5), las=2) #tick marks for y axis
      ...
      #add legend
      legend(-1,-5,legend=c('Aligned','Small RDM', 'Large RDM'),
             col=c(colourscheme[['aligned']][['S']],colourscheme[['sml']][['S']],colourscheme[['lrg']][['S']]),
             lty=1,bty='n',cex=1,lwd=2)
      
    } else if (ptype == 'mir'){
      groups = c('alnmir', 'mir_sml', 'mir_lrg')
      # create plot
      meanGroupReaches <- list() #empty list so that it plots the means last
      ...
      #add legend
      legend(-1,-5,legend=c('Aligned','Small MIR', 'Large MIR'),
             col=c(colourscheme[['aligned']][['S']],colourscheme[['sml']][['S']],colourscheme[['lrg']][['S']]),
             lty=1,bty='n',cex=1,lwd=2)
